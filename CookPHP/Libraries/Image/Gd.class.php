...
class Gd {
    ...
    public function water($source, $locate = Image::IMAGE_WATER_SOUTHEAST, $alpha = 80) {
        //资源检测
        if (empty($this->img)) {
            Error::showError('没有可以被添加水印的图像资源');
        }
        if (!is_file($source)) {
            Error::showError('水印图像不存在');
        }

        //获取水印图像信息
        $info = getimagesize($source);
        if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
            Error::showError('非法水印文件');
        }
    ...
    public function text($text, $font, $size, $color = '#00000000', $locate = Image::IMAGE_WATER_SOUTHEAST, $offset = 0, $angle = 0) {
        //资源检测
        if (empty($this->img)) {
            Error::showError('没有可以被写入文字的图像资源');
        }
        if (!is_file($font)) {
            Error::showError("不存在的字体文件：{$font}");
        }
