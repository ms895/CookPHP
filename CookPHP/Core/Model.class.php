...
class Model {
    ...
    protected $data;
    protected $partition = [];
    private static $_driver = [], $_cache = false;
    ...
    public function setConfig($config = []) {
        $this->config = !empty($this->config) ? array_merge($this->config, $config ?: Config::get('database')) : ($config ?: Config::get('database'));
    ...
    public function setTable($table) {
        ...
        $this->table = Application::parseName($table);
        $this->table();
        return $this;
    }
    ...
    public function connect() {
        ...
        if (!isset(self::$_driver[$this->config['driver']][$this->config['host']][$this->config['port']])) {
            self::$_driver[$this->config['driver']][$this->config['host']][$this->config['port']] = Loader::initialize(__NAMESPACE__ . '\\Db\\' . ucfirst($this->config['driver']));
            self::$_driver[$this->config['driver']][$this->config['host']][$this->config['port']]->setConfig($this->config);
            !self::$_driver[$this->config['driver']][$this->config['host']][$this->config['port']]->enabled() && Error::showDbError('Error Database Handler', ucfirst($this->config['driver']), 'db');
            $this->connected = self::$_driver[$this->config['driver']][$this->config['host']][$this->config['port']]->connect();
        }
        $this->driver = self::$_driver[$this->config['driver']][$this->config['host']][$this->config['port']] ?? Error::showDbError('Error Database Handler', ucfirst($this->config['driver']), 'db');
        return $this->driver;
    }
    ...
    public function getTableName() {
        ...
        empty($this->params['table']) && Error::showDbError('Table Name Required', '', 'db');
    ...
    public function strSql($string) {
        ...
        $string = preg_replace_callback("/__([A-Z0-9_-]+)__/sU", function ($match) {
     ...
     * @return array
     */
    public function fetch($sql = null, $duration = '', $name = null) {
        ...
        if (is_string(self::$_cache)) {
            $duration = self::$_cache;
        } elseif (is_numeric(self::$_cache)) {
            $duration = (int) self::$_cache;
        }
        if (is_string($duration)) {
            switch ($duration) {
                ...
                case 'h'://1小时
                    ...
                    break;
                default:
                    $duration = (int) $this->config['cacheexpire'];
            }
        }
        $this->params = [];
        return \Libraries\Cache::init($this->config['cachedriver'] ?? 'file')->remember((string) $sql, function () use ($sql) {
                    return $this->fetchFromDb($sql);
                }, (int) $duration);
    }
     ...
     * @return $this
     */
    public function cacheOn($time = true) {
        self::$_cache = $time;
        return $this;
    }
     ...
     * @return array
     */
    public function read($sql = null, $duration = '', $name = null) {
     ...
     * @return array
     */
    public function fetchAll($sql = null, $duration = null, $name = null) {
        return $this->fetch($sql, $duration, $name);
    }
     ...
     * @return array
     */
    public function fetchAssoc($sql = null, $duration = null, $name = null) {
...
//        }
     ...
     * @return bool
     */
    public function save($replace = false, $table = '', $data = []) {
        if (empty($table)) {
            $table = $this->getTableName();
        }
        if (empty($data)) {
     ...
     * @return bool
     */
    public function create($replace = false, $table = '', $data = []) {
    ...
    public function update($table = '', $data = [], $conditions = [], $details = []) {
        ...
        return $this->query($query) ? $this->lastAffected() : false;
    }
