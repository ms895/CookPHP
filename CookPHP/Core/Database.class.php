...
abstract class Database {
    ...
    public function renderStatement($type, $data) {
        ...
        switch (strtolower($type)) {
            case 'select':
                ...
                $sql = str_replace(['%FIELD%', '%TABLE%', '%ALIASES%', '%JOIN%', '%WHERE%', '%GROUP%', '%ORDER%', '%LIMIT%'], [$data['fields'] ?? '*', $data['table'], $aliases, $data['joins'] ?? '', $data['conditions'] ?? '', $data['group'] ?? '', $data['order'] ?? '', $data['limit'] ?? ''], $this->selectSql);
                break;
            case 'create':
            case 'insert':
                $values = $data['values'];
    ...
    public function __destruct() {
        ...
        if ($this->lastError()) {
            Error::showDbError('Error SQL', $this->logSqlError(), 'db');
        }
    }
