...
namespace Library\Template\Cook;
...
class Cook {
    ...
    private function compile(string $template, string $compileFile) {
        ...
        if (!F::has($compileFile) || !Config::get('template.compilecache') || ($md5 = md5($content)) !== F::get($compileFile, 8, 32)) {
            if (!empty($content)) {
                $this->compileCode($content);
                ($this->compresshtml || Config::get('template.compresshtml')) && $this->compressHtml($content);
            }
    ...
    private function _replace() {
        ...
        $this->_replace[] = '';
        $this->_replace[] = '';
        $this->_replace[] = '<?php echo config("\\1");?>';
        $this->_replace[] = '<?php echo url("\\1");?>';
        $this->_replace[] = '<?php echo language("\\1");?>';
