...
class Url {
    ...
    public static function parse(string $url, $params = [], $domain = false): string {
        ...
        if (empty($array['path']) || strpos($array['path'], '/') === false) {
            $array['path'] = Action::getController() . '/' . (!empty($array['path']) ? $array['path'] : Action::getMethod()) . (Config::get('route.htmlsuffix') ? '.' . Config::get('route.htmlsuffix') : '');
        }

        $url = !empty($array['path']) ? $array['path'] : Action::getController() . '/' . Action::getMethod() . (Config::get('route.htmlsuffix') ? '.' . Config::get('route.htmlsuffix') : '');
        ...
        if (isset($array['scheme'])) {
            ...
            if (!Config::get('route.domain')) {
                if (substr($url, 0, 1) !== '/') {
                    $url = (strtolower(Config::get('route.project')) === strtolower(Action::getRoute()) ? '' : Action::getRoute() . '/') . $url;
                }
            }
        }
    ...
    public static function filter($str) {
        !empty($str) && !empty(Config::get('route.permittedurichars')) && !preg_match("|^[" . str_replace(['\\-', '\-'], '-', preg_quote(Config::get('route.permittedurichars'), '-')) . "]+$|i", $str) && Exception::show404();
